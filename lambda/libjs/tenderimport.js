const { textExclusion, textExclusionIfNoCpv, cpvParseTreat, textParseTreat } = require('./textparse')
...
exports.importTender = (tender, CpvList, textParses) => {
  ...
  // Test exclusion
  let isOk = textExclusion(tender.title, 'TITLE')
  if (!isOk.status) {
    importOrigine.exclusion = 'TITLE'
    importOrigine.exclusionWord = isOk.origine
    importOrigine.status = -10
    return({importOrigine})
  }
  isOk = textExclusion(tender.description, 'DESCRIPTION')
  if (!isOk.status) {
  ...
  const dataSearchCpvCriterions = this.searchCpvCriterions(tender, CpvList, textParses)
...
exports.searchCpvCriterions = (tender, CpvList, textParses) => {
  ...
  const tenderCriterionCpvsTitle = cpvParseTreat(tender.title, CpvList, true, 'TITLE')
  if (tenderCriterionCpvsTitle.length) {
    const isOk = textExclusionIfNoCpv(tender.title, 'TITLE')
    if (!isOk.status) {
  ...
  const tenderCriterionCpvsDescription = cpvParseTreat(tender.description, CpvList, true, 'DESCRIPTION')
  ...
  const tenderCriterionsTitle = textParseTreat(tender.title, textParses, 'TITLE')
  const tenderCriterionsDescription = textParseTreat(tender.description, textParses, 'DESCRIPTION')
